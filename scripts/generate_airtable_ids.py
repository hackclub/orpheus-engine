...
def _load_existing_base_blocks(output_path: Path) -> dict[str, list[str]]:
    """
    Parse an existing generated_ids.py into a dict of base_class_name -> lines.

    Each value is the full indented block for that base class (including the
    `    class Foo:` header line through the blank line that follows it),
    ready to be spliced back into the new file verbatim if a fetch fails.
    """
    ...
    # Find top-level base class blocks (indented exactly 4 spaces inside AirtableIDs)
    i = 0
    while i < len(lines):
        line = lines[i]
        ...
        if m:
            ...
            block = []
            # Collect lines until the next top-level class or end of AirtableIDs
            j = i
            while j < len(lines):
                next_line = lines[j]
                # Stop when we hit the next top-level class (but not the first line)
                if j > i and re.match(r'^    class \w+:\s*$', next_line):
                    break
                # Stop at the closing fmt: on marker
                if next_line.strip() == "# fmt: on":
                    break
                block.append(next_line.rstrip('\n'))
                j += 1
            blocks[base_class_name] = block
            i = j
        else:
            i += 1
...
def generate_ids(api: Api, config: AirtableServiceConfig, output_path: Path):
    ...
    existing_blocks = _load_existing_base_blocks(output_path)
    if existing_blocks:
        log.info(f"Loaded existing blocks for {len(existing_blocks)} base(s) as fallback for fetch failures.")
    ...
    for base_key, base_config in config.bases.items():
        ...
        try:
            ...
            if base_class_name in existing_blocks:
                log.warning(f"  Preserving existing block for '{base_class_name}' from previous run.")
                generated_code.extend(existing_blocks[base_class_name])
            else:
                log.warning(f"  No existing block found for '{base_class_name}' — skipping entirely.")
